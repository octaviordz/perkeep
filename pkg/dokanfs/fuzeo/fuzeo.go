...
func (c *Conn) ReadRequest() (Request, error) {
	...
	if n < inHeaderSize {
		putMessage(m)
		return nil, errors.New("fuzeo: message too short")
	}
	...
	if m.hdr.Len != uint32(n) {
		...
		err := fmt.Errorf("fuzeo: read %d opcode %d but expected %d", n, m.hdr.Opcode, m.hdr.Len)
		putMessage(m)
		return nil, err
	}
	...
	case opBmap:
		...
		// uint64. On Linux, it triggers only with blkdev fuzeo mounts,
		// that claim to be backed by an actual block device. FreeBSD
		// seems to send it for just any fuzeo mount, whether there's a
