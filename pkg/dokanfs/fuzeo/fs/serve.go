...
package fs // import "perkeep.org/pkg/dokanfs/fuzeo/fs"
...
//
// Other fuzeo requests can be handled by implementing methods from the
...
type FSStatfser interface {
	...
	Statfs(ctx context.Context, req *fuzeo.StatfsRequest, resp *fuzeo.StatfsResponse) error
}

type FSDestroyer interface {
	// Destroy is called when the file system is shutting down.
	//
	// Linux only sends this request for block device backed (fuzeoblk)
...
type FSInodeGenerator interface {
	...
	//
	// Not all filesystems bother tracking inodes, but fuzeo requires
...
type Node interface {
	...
	Attr(ctx context.Context, attr *fuzeo.Attr) error
}
...
type NodeGetattrer interface {
	...
	Getattr(ctx context.Context, req *fuzeo.GetattrRequest, resp *fuzeo.GetattrResponse) error
}
...
type NodeSetattrer interface {
	...
	Setattr(ctx context.Context, req *fuzeo.SetattrRequest, resp *fuzeo.SetattrResponse) error
}
...
type NodeSymlinker interface {
	...
	Symlink(ctx context.Context, req *fuzeo.SymlinkRequest) (Node, error)
}

// This optional request will be called only for symbolic link nodes.
type NodeReadlinker interface {
	// Readlink reads a symbolic link.
	Readlink(ctx context.Context, req *fuzeo.ReadlinkRequest) (string, error)
}

type NodeLinker interface {
	// Link creates a new directory entry in the receiver based on an
	// existing Node. Receiver must be a directory.
	Link(ctx context.Context, req *fuzeo.LinkRequest, old Node) (Node, error)
}
...
type NodeRemover interface {
	...
	Remove(ctx context.Context, req *fuzeo.RemoveRequest) error
}
...
type NodeAccesser interface {
	...
	Access(ctx context.Context, req *fuzeo.AccessRequest) error
}
...
type NodeRequestLookuper interface {
	...
	Lookup(ctx context.Context, req *fuzeo.LookupRequest, resp *fuzeo.LookupResponse) (Node, error)
}

type NodeMkdirer interface {
	Mkdir(ctx context.Context, req *fuzeo.MkdirRequest) (Node, error)
}
...
type NodeOpener interface {
	...
	Open(ctx context.Context, req *fuzeo.OpenRequest, resp *fuzeo.OpenResponse) (Handle, error)
}

type NodeCreater interface {
	// Create creates a new directory entry in the receiver, which
	// must be a directory.
	Create(ctx context.Context, req *fuzeo.CreateRequest, resp *fuzeo.CreateResponse) (Node, Handle, error)
}
...
type NodeRenamer interface {
	Rename(ctx context.Context, req *fuzeo.RenameRequest, newDir Node) error
}

type NodeMknoder interface {
	Mknod(ctx context.Context, req *fuzeo.MknodRequest) (Node, error)
}

// TODO this should be on Handle not Node
type NodeFsyncer interface {
	Fsync(ctx context.Context, req *fuzeo.FsyncRequest) error
}
...
type NodeGetxattrer interface {
	...
	// node.
	//
	// If there is no xattr by that name, returns fuzeo.ErrNoXattr.
	Getxattr(ctx context.Context, req *fuzeo.GetxattrRequest, resp *fuzeo.GetxattrResponse) error
}

type NodeListxattrer interface {
	// Listxattr lists the extended attributes recorded for the node.
	Listxattr(ctx context.Context, req *fuzeo.ListxattrRequest, resp *fuzeo.ListxattrResponse) error
}

type NodeSetxattrer interface {
	// Setxattr sets an extended attribute with the given name and
	// value for the node.
	Setxattr(ctx context.Context, req *fuzeo.SetxattrRequest) error
}
...
type NodeRemovexattrer interface {
	...
	//
	// If there is no xattr by that name, returns fuzeo.ErrNoXattr.
	Removexattr(ctx context.Context, req *fuzeo.RemovexattrRequest) error
}

var startTime = time.Now()

func nodeAttr(ctx context.Context, n Node, attr *fuzeo.Attr) error {
...
type HandleFlusher interface {
	...
	Flush(ctx context.Context, req *fuzeo.FlushRequest) error
}
...
type HandleReadDirAller interface {
	ReadDirAll(ctx context.Context) ([]fuzeo.Dirent, error)
}
...
type HandleReader interface {
	...
	Read(ctx context.Context, req *fuzeo.ReadRequest, resp *fuzeo.ReadResponse) error
}
...
type HandleWriter interface {
	...
	Write(ctx context.Context, req *fuzeo.WriteRequest, resp *fuzeo.WriteResponse) error
}

type HandleReleaser interface {
	Release(ctx context.Context, req *fuzeo.ReleaseRequest) error
}
...
type HandlePoller interface {
	...
	// regular files.
	Poll(ctx context.Context, req *fuzeo.PollRequest, resp *fuzeo.PollResponse) error
}
...
type HandleLocker interface {
	...
	QueryLock(ctx context.Context, req *fuzeo.QueryLockRequest, resp *fuzeo.QueryLockResponse) error
}
...
type HandleFAllocater interface {
	...
	//
	// Note that the kernel limits what modes are acceptable in any fuzeo filesystem.
	FAllocate(ctx context.Context, req *fuzeo.FAllocateRequest) error
}

type Config struct {
	// Function to send debug log messages to. If nil, use fuzeo.Debug.
	// Note that changing this or fuzeo.Debug may not affect existing
	// calls to Serve.
	//
	// See fuzeo.Debug for the rules that log functions must follow.
	...
	//
	// Must not retain req.
	WithContext func(ctx context.Context, req fuzeo.Request) context.Context
}

// New returns a new fuzeo server ready to serve this kernel fuzeo
// connection.
//
// Config may be nil.
func New(conn *fuzeo.Conn, config *Config) *Server {
	s := &Server{
		conn:         conn,
		req:          map[fuzeo.RequestID]*serveRequest{},
		nodeRef:      map[Node]fuzeo.NodeID{},
		notifyWait:   map[fuzeo.RequestID]chan<- *fuzeo.NotifyReply{},
...
type Server struct {
	...
	conn    *fuzeo.Conn
	debug   func(msg interface{})
	context func(ctx context.Context, req fuzeo.Request) context.Context
	...
	nodeRef    map[Node]fuzeo.NodeID
	handle     []*serveHandle
	freeNode   []fuzeo.NodeID
	freeHandle []fuzeo.HandleID
	nodeGen    uint64

	// pending notify upcalls to kernel
	notifyMu   sync.Mutex
	notifySeq  fuzeo.RequestID
	notifyWait map[fuzeo.RequestID]chan<- *fuzeo.NotifyReply
...
// Serve serves the fuzeo connection by making calls to the methods
...
func (s *Server) Serve(fs FS) error {
	...
	handleEntry := xfz.MountHandleMap()[fd]

	//TODO(ORC): Mount handle.
	_, err := xfz.Mount(&dokan.Config{Path: handleEntry.Dir})
	if err != nil {
		return err
	}
...
// Serve serves a fuzeo connection with the default settings. See
// Server.Serve.
func Serve(c *fuzeo.Conn, fs FS) error {
	server := New(c, nil)
	return server.Serve(fs)
}

type serveRequest struct {
	Request fuzeo.Request
	cancel  func()
}
...
func (sn *serveNode) attr(ctx context.Context, attr *fuzeo.Attr) error {
...
func (c *Server) saveNode(inode uint64, node Node) (id fuzeo.NodeID, gen uint64) {
	c.meta.Lock()
	defer c.meta.Unlock()
...
func (c *Server) saveHandle(handle Handle) (id fuzeo.HandleID) {
	c.meta.Lock()
	...
	if n := len(c.freeHandle); n > 0 {
		...
		id = fuzeo.HandleID(len(c.handle))
...
func (c *Server) dropNode(id fuzeo.NodeID, n uint64) (node Node, forget bool) {
	c.meta.Lock()
	defer c.meta.Unlock()
	snode := c.node[id]
...
func (c *Server) dropHandle(id fuzeo.HandleID) {
	c.meta.Lock()
	c.handle[id] = nil
...
type missingHandle struct {
	Handle    fuzeo.HandleID
	MaxHandle fuzeo.HandleID
}
...
func (c *Server) getHandle(id fuzeo.HandleID) (shandle *serveHandle) {
	c.meta.Lock()
	defer c.meta.Unlock()
	if id < fuzeo.HandleID(len(c.handle)) {
		shandle = c.handle[uint(id)]
	}
	if shandle == nil {
		c.debug(missingHandle{
			Handle:    id,
			MaxHandle: fuzeo.HandleID(len(c.handle)),
		})
	}
	return
}
...
type logMissingNode struct {
	MaxNode fuzeo.NodeID
}

func opName(req fuzeo.Request) string {
...
var _ fuzeo.ErrorNumber = handlerPanickedError{}

func (h handlerPanickedError) Errno() fuzeo.Errno {
	if err, ok := h.Err.(fuzeo.ErrorNumber); ok {
		return err.Errno()
	}
	return fuzeo.DefaultErrno
}
...
var _ fuzeo.ErrorNumber = handlerTerminatedError{}

func (h handlerTerminatedError) Errno() fuzeo.Errno {
	return fuzeo.DefaultErrno
}
...
var _ fuzeo.ErrorNumber = handleNotReaderError{}

func (e handleNotReaderError) Errno() fuzeo.Errno {
	return fuzeo.Errno(syscall.ENOTSUP)
}

func initLookupResponse(s *fuzeo.LookupResponse) {
...
func (c *Server) serve(r fuzeo.Request) {
	...
	if id := hdr.Node; id != 0 {
		...
		if snode == nil {
			...
			c.debug(response{
				...
				Error:   fuzeo.Errno(err).ErrnoName(),
				...
				Out: logMissingNode{
					MaxNode: fuzeo.NodeID(len(c.node)),
				},
			})
			r.RespondError(err)
			return
	...
	// with the same ID and be very confuzeod.
		...
		if err, ok := resp.(error); ok {
			errno := fuzeo.ToErrno(err)
			msg.Errno = errno.ErrnoName()
			if errno != err && syscall.Errno(errno) != err {
				// if it's more than just a fuzeo.Errno or a
	...
	defer func() {
		if rec := recover(); rec != nil {
			...
			buf = buf[:n]
			log.Printf("fuzeo: panic in handler for %v: %v\n%s", r, rec, buf)
